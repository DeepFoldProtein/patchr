...
class AlignmentMixin:
    def get_residue_mapping_dna_rna(self, atoms: List[Dict], seqres_sequence: str, final_sequence: str = None) -> Dict[int, int]:
        ...
        if final_sequence and final_sequence != seqres_sequence:
            ...
            debug("Custom sequence detected for DNA/RNA:")
            detail(f"- SEQRES length: {seq_length}")
            detail(f"- Final sequence length: {final_seq_length}")
            detail(f"- Structure residues: {num_structure_residues}")
            ...
            debug(f"Best alignment offset: {best_offset} (matches: {max_matches}/{min(seq_length, final_seq_length)})")
            ...
            info(f"Structure residues will be mapped to sequence positions {best_offset + 1} to {best_offset + num_structure_residues}")
            ...
            if first_residue_num == 1 and last_residue_num <= seq_length:
                ...
                if missing_in_range:
                    warning(f"Missing residues in structure: {sorted(missing_in_range)}")
    ...
    def get_residue_mapping(self, atoms: List[Dict], seqres_sequence: str, final_sequence: str, chain_id: Optional[str] = None) -> Dict[int, int]:
        ...
        Uses Ca coordinates to determine structural order, ignoring insertion codes.
        ...
        Returns:
            Dict mapping sequential structure index to sequence position (1-based)
        """
        # First, extract residues with their Ca coordinates for structural ordering
        ...
        for atom_idx, atom in enumerate(atoms):
            ...
            # Store Ca coordinates for structural ordering (preferred)
        ...
        # Use Ca if available, otherwise use average of all atoms
        for residue_key, info_val in residue_info.items():
            coords_list = []
            ca_coords = None

            for atom in info_val['atoms']:
                try:
                    coords = (
                        float(atom['Cartn_x']),
                        float(atom['Cartn_y']),
            ...
            # Use Ca if available, otherwise use average of all atoms
            if ca_coords:
                info_val['center_coords'] = ca_coords
            elif coords_list:
                ...
                info_val['center_coords'] = (avg_x, avg_y, avg_z)
            else:
                info_val['center_coords'] = None
        ...
        for idx in struct_residue_order:
            ...
            if one_letter is None and chain_id and chain_id in self.non_standard_residues:
                ...
                info_val = residue_info.get(key, {})
                atoms_list = info_val.get('atoms', [])
        ...
        if (len(struct_residue_order) == len(seqres_sequence)
            ...
            info(f"Using 1:1 sequential mapping for all-non-standard chain ({len(residue_mapping)} residues)")
            if residue_mapping:
                mapped_positions = sorted(residue_mapping.values())
                info(f"Mapped to sequence positions: {min(mapped_positions)} to {max(mapped_positions)}")
                info(f"Original residue range: {struct_residue_order[0]} to {struct_residue_order[-1]}")
            return residue_mapping
        ...
        if not alignments:
            error("Alignment failed, cannot create mapping")
            return {}
        ...
        # For synthetic chains (e.g. A-2), look up using the base chain ID
        ...
        if (chain_id and _mods_lookup_id in getattr(self, '_modifications_from_entity_poly', {}) and
            ...
            if len(struct_to_seqres_map) < len(struct_residue_order) or struct_seq == seqres_sequence:
                ...
                info(f"Using 1:1 sequential mapping for modification chain {chain_id} ({len(struct_to_seqres_map)} residues)")
        ...
        # Typical Ca-Ca distance in proteins is ~3.8 A
        # If two consecutive structure residues are close (< 6 A), they should map to nearby SEQRES positions
                ...
                if next_pos <= len(seqres_sequence):
                    ...
                    if struct_res_2 == seqres_res_next and next_pos not in reverse_map:
                        ...
                        continuity_issues.append({
                            ...
                            'reason': f'Structurally consecutive residues (distance {struct_distance:.1f}A) must map consecutively'
                        })
                        continue
                            ...
                            if struct_res_2 == seqres_res:
                                ...
                                if nearby_pos not in reverse_map:  # Not already mapped
                                    continuity_issues.append({
                                        ...
                                        'residue': struct_res_2,
                                        'reason': f'Structurally consecutive residues (distance {struct_distance:.1f}A, gap {seqres_gap}) should map closer'
                                    })
                                    found_better = True
                                    break
        ...
        while continuity_issues and iteration < max_iterations:
            ...
            if iteration == 1:
                info(f"Fixing {len(continuity_issues)} mapping issue(s) caused by repetitive sequences")
            else:
                detail(f"Iteration {iteration}: Fixing {len(continuity_issues)} more issue(s)")

            for issue in continuity_issues:
                detail(f"- Seq idx {issue['seq_idx']} ({issue['residue']}): {issue['old_seqres']} -> {issue['new_seqres']} ({issue['reason']})")
        ...
        if total_fixed > 0:
            info(f"Total fixed: {total_fixed} mapping issue(s)")
        ...
        if (self.uniprot_mode or self.interactive_sequence or _has_custom_seq) and final_sequence != seqres_sequence:
            ...
            if seqres_start_in_target >= 0:
                ...
                info(f"SEQRES is an exact substring of {sequence_type} (position {seqres_start_in_target + 1}-{seqres_start_in_target + len(seqres_sequence)})")
                ...
                if alignment is None:
                    warning(f"Could not align SEQRES with {sequence_type}, using SEQRES positions")
        ...
        info(f"Found {len(residue_mapping)} residues in structure")
        if residue_mapping:
            mapped_positions = sorted(residue_mapping.values())
            info(f"Mapped to sequence positions: {min(mapped_positions)} to {max(mapped_positions)}")
            info(f"Original residue range: {min(struct_residue_order)} to {max(struct_residue_order)}")
    ...
    def filter_atoms_by_uniprot_sequence(self, atoms: List[Dict], residue_mapping: Dict[int, int], final_sequence: str) -> Tuple[List[Dict], List[Dict]]:
        ...
        info(f"Structure sequence length: {len(struct_seq)}")
        info(f"Target sequence length: {len(target_seq)} ({sequence_type})")
        info("Aligning sequences...")

        # Align sequences
        alignment = self.align_sequences(struct_seq, target_seq)
        if not alignment:
            warning("Could not align sequences, skipping filtering")
            return atoms, []

        aligned_struct, aligned_target, score, begin, end = alignment

        info(f"Alignment score: {score}")
        info(f"Aligned length: {len(aligned_struct)}")
        ...
        info(f"Matches: {matches}, Mismatches: {mismatches}")
