...
def _categorize_job(rec: dict) -> str:
...
async def stats_daily(days: int = 30):
    ...
    keeps the most recent N calendar days (0 = all)."""
    def _blank():
    ...
    for rec in _load_all_jobs():
        d = str(rec.get("created_at", ""))[:10]
        if not d:
            continue
        cat = _categorize_job(rec)
        if cat == "other":
            continue
        st = rec.get("status")
    ...
    for r in _read_request_records():
        d = str(r.get("ts", ""))[:10]
        if not d:
            continue
        b = _day(d)["requests"]
