...
def _is_valid_checkpoint(path: Path) -> bool:
    """Check if a checkpoint file is a valid zip archive (PyTorch format)."""
    try:
...
def _safe_download(url: str, dest: Path) -> None:
    """Download a file atomically using a temp file to prevent corruption."""
...
def _ensure_checkpoint(dest: Path, urls: list[str], label: str) -> None:
    """Ensure a valid checkpoint exists, downloading if needed."""
    if dest.exists():
        if _is_valid_checkpoint(dest):
            return
        click.echo(
            f"Existing {label} at {dest} is corrupted. Re-downloading..."
        )
        dest.unlink()

    click.echo(
        f"Downloading {label} to {dest}. You may "
    ...
    for i, url in enumerate(urls):
        try:
            ...
            if i == len(urls) - 1:
                msg = f"Failed to download {label} from all URLs. Last error: {e}"
                raise RuntimeError(msg) from e
            continue

    if not _is_valid_checkpoint(dest):
        dest.unlink(missing_ok=True)
        msg = f"Downloaded {label} is corrupted. Please try again or download manually."
...
def download_boltz1(cache: Path) -> None:
    ...
    if not ccd.exists():
        ...
        _safe_download(CCD_URL, ccd)

    # Download model
    model = cache / "boltz1_conf.ckpt"
    _ensure_checkpoint(model, BOLTZ1_URL_WITH_FALLBACK, "Boltz-1 model weights")
...
def download_boltz2(cache: Path) -> None:
    ...
    if not tar_mols.exists():
        ...
        _safe_download(MOL_URL, tar_mols)
    if not mols.exists():
        click.echo(
    ...
    _ensure_checkpoint(model, BOLTZ2_URL_WITH_FALLBACK, "Boltz-2 model weights")

    # Download affinity model
    affinity_model = cache / "boltz2_aff.ckpt"
    _ensure_checkpoint(
        affinity_model, BOLTZ2_AFFINITY_URL_WITH_FALLBACK, "Boltz-2 affinity weights"
    )
