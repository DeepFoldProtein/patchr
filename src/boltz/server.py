...
async def upload_structure(
    ...
    downloading from PDB. PDB files are automatically converted to mmCIF.
    """
    ...
    if cif_file.filename.endswith(".pdb"):
        try:
            ...
            print(f"  Converted PDB → mmCIF: {cif_file.filename} → {cif_filename}")
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Failed to convert PDB to mmCIF: {str(e)}")
    else:
        cif_filename = cif_file.filename

    # Save file (always as .cif after potential conversion)
