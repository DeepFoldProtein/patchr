...
class SampleDictToFeatures:
    ...
    def get_feature_dict(self) -> tuple[dict[str, torch.Tensor], AtomArray, TokenArray]:
        ...
        # ── Inpainting template features ──────────────────────────────────────
        inpainting_cfg = self.input_dict.get("inpainting")
        if inpainting_cfg:
            feature_dict = self._add_inpainting_features(
                feature_dict, atom_array, inpainting_cfg
            )

        return feature_dict, atom_array, token_array

    def _add_inpainting_features(
        self,
        feature_dict: dict,
        atom_array: AtomArray,
        inpainting_cfg: dict,
    ) -> dict:
        """Compute and add inpainting template features to feature_dict.

        Reads template_cif and metadata from inpainting_cfg and maps template
        atom coordinates onto the atom_array.  Adds two entries:
            - "inpainting_template_coords" : Tensor [N_atom, 3]
            - "inpainting_template_mask"   : Tensor [N_atom], int64

        Args:
            feature_dict: existing feature dict to augment.
            atom_array: Protenix AtomArray for the current sample.
            inpainting_cfg: the "inpainting" sub-dict from the input JSON, e.g.
                {
                    "template_cif": "path/to/template.cif",
                    "metadata": "path/to/metadata.json",
                    "chain_id_mapping": {"A": "AA", "B": "AB", ...}
                }

        Returns:
            Updated feature_dict with inpainting keys added.
        """
        from protenix.data.inpainting.template_featurizer import (
            compute_inpainting_template_features,
        )
        ...
        coords, mask = compute_inpainting_template_features(
            atom_array=atom_array,
            template_cif_path=template_cif,
            metadata_path=metadata,
            chain_id_mapping=chain_id_mapping,
        )

        feature_dict["inpainting_template_coords"] = torch.from_numpy(coords)
        feature_dict["inpainting_template_mask"] = torch.from_numpy(mask)
        return feature_dict
