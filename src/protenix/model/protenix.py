...
class Protenix(nn.Module):
    ...
    def sample_diffusion(self, **kwargs: Any) -> torch.Tensor:
        ...
        # Inpainting boundary refinement configs (optional, fall back to defaults)
        ...
        for key, default in [
            ...
            if key not in kwargs:
                _configs[key] = inpainting_cfg.get(key, default)
    ...
    def _main_inference_loop(
        ...
        # Center inpainting template coords (no-op when no inpainting keys present)
        self._prepare_inpainting(input_feature_dict)
